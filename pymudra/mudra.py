...
class MUDRAEstimator():
    ...
    def predict(self, X):
        ...
        ## average nearest neighbor activities for each descriptor 
        ## weighted by similarity to query molecule
        sims = 1 - dists
        sims_sum = np.sum(sims, axis = 0)
        normalized_sims = sims / sims_sum
        y_pred = np.sum(normalized_sims * preds, axis = 0)
